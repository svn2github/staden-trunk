...
proc FindOligos { io } {
    ...
    set f [keylget gap5_defs FINDOLIGO.WIN]
    ...
    lorf_in $f.infile [keylget gap5_defs FINDOLIGO.INFILE] \
    ...
    ###########################################################################
    #mis-match scale
    keylset mm MAXMIS [keylget gap5_defs FINDOLIGO.MAXMIS]
    ...
    keylset sm SELMODE [keylget gap5_defs FINDOLIGO.SELMODE]
    keylset ms MATCHSEQ [keylget gap5_defs FINDOLIGO.MATCHSEQ]
	...
	-default [keylget gap5_defs FINDOLIGO.CONSONLY.VALUE]

    radiolist $f.hidden_data \
	-orient horizontal \
	-title "Use hidden data" \
	-buttons {No Yes} \
	-default [keylget gap5_defs FINDOLIGO.CUTOFFS.VALUE]

    radiolist $f.sel_mode.l.rl \
	    -title [keylget sm SELMODE.NAME] \
	    -orient vertical\
	    -default [expr {[keylget sm SELMODE.VALUE]-1}]\
	    -buttons [format { \
	    { %s -command { \
