	...
	if {[string match "##graphic" $line]} {
		...
		1 {
			...
			[emboss create \
			     ...
			     -graph 0 \
			     -data $f]
		    set result_id($id_count) [lindex $res($id_count) 0]
		    set result($id_count) [lindex $res($id_count) 1]

		    if {$result_id($id_count) != -1} {
			plot_emboss $seq_id(0) $result_id($id_count) $result($id_count) $program
		    }
		}
		2 {
		    set res($id_count) \
			...
			plot_emboss_dot $seq_id(0) $seq_id(1) \
				$result_id($id_count) $result($id_count) $program
		    }
		}
		0 {
		    # Do nothing
...
proc emboss_init {} {
    ...
    lappend auto_path $env(STADLIB)/spin2_emboss/acdtcl
