		       ...
		       int seq_num)
{
    ...
    Tcl_VarEval(result->interp, "result_list_update ", NULL);
		 ...
		 int wy)
{
	...
	seq_disp_result->cursor[HORIZONTAL] = create_cursor(GetSeqNum(seq_id_h), 1, NULL, line_width, 1, HORIZONTAL, wx);
    ...
    if (cursor_id_v < 0) {
	/* no vertical cursor */
	seq_disp_result->cursor[VERTICAL] = create_cursor(GetSeqNum(seq_id_v), 1, NULL, line_width, 1, VERTICAL, wy);
...
{ 
   ...
   /* yypp */
    ...
    if (NULL == (score_display = (int **)xmalloc(MAX_SCORE_MATRIX *sizeof(int *))))
	return -1;
    for (i = 0; i < MAX_SCORE_MATRIX; i++) {
	if (NULL == (score_display[i] = (int *) xmalloc(MAX_SCORE_MATRIX *
							   sizeof(int))))
	    return -1;  
    }
				...
				int result_index)
{
    ...
    seq_disp_win = get_default_string(interp, spin_defs, "SEQ_DISP.WIN");
