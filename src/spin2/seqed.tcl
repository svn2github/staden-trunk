...
############################################################
	...
	-ok_command "SeqedDisplay_d2 $t $t.seq_id $pos"\
...
proc SeqedDisplay_d2 {t seq_id pos} {
    global HORIZONTAL

    set id [name_to_seq_id [seq_id_name $seq_id]]

    SeqedDisplay -2 $pos $id 
...
#packed seqed
proc SeqedDisplayPacked {pos seq_id win row column} {
    ...
    set win [frame $win.ed$se_id]
    create_seqed $win $win.se $pos $seq_id

    grid $win -row $row -column $column -sticky ew

    #fill in widget info and register
    set seqed_id [seqed_display -window $win.se -seq_id $seq_id -pos $pos]
...
#toplevel seqed
proc SeqedDisplay {c_id pos seq_id} {
    global se_id tk_utils_defs spin_defs HORIZONTAL TOP_S
...
#    fix_maxsize_text $f $font_width $font_height 40 80
    ...
    set strand $TOP_S
    set element_info [create_seq_element $seq_id -1 "" $strand 0 $HORIZONTAL "X" SEQED [keylget spin_defs CONTAINER.TITLE] 0 0 $c_id]

    set f [keylget element_info container_win]
    ...
    create_seqed $c_win $c_win$e_win $pos $seq_id $c_id $e_id 

    # Trap window quit so that clean-up is done properly
    #wm protocol $f WM_DELETE_WINDOW "seqed_quit $f $f.se $f.REnz"

    #fill in widget info and register
    set seqed_id [seqed_display -window $c_win$e_win -seq_id $seq_id -pos $pos -container_id $c_id -container_win $c_win -element_id $e_id]
...
proc create_seqed {c_win se pos seq_id c_id e_id} {
    set w $c_win
    global $w.Ruler $w.Complement DNA tk_utils_defs seqed_menu env

    set $w.Ruler 1
    set $w.Complement 0

    frame $c_win.f$e_id -bd 2 -relief groove
    seqed $se -xscrollcommand "$w.esb_h set" -yscrollcommand "$w.esb_v set" \
    ...
    scrollbar $w.esb_h -orient horizontal -command "$se xview"
    scrollbar $w.esb_v -orient vertical -command "$se yview"

    #seqednames $w.names -width 23 -height 1 -bd 2 -relief raised
 
    frame $w.edbuttons
    container_menubar $c_win $c_id $seqed_menu
    set_element_specs $se "menu [list $seqed_menu]"

   # pack $f.names -side left -expand yes

    grid_insert $c_win row [keylget tk_utils_defs CONTAINER.SEQ_EDITOR_ROW] 2
    set row [keylget tk_utils_defs CONTAINER.SEQ_EDITOR_ROW]
    set column [keylget tk_utils_defs CONTAINER.RULER_COL]

    grid columnconfig $w $column -weight 1
    grid rowconfig $w $row -weight 1

    #grid $w.buttons -row 0 -column 0 -sticky ew
    #grid $se -row 1 -column 0 -sticky nsew

    grid $c_win.f$e_id -row $row -column $column -sticky nsew
    pack $c_win.f$e_id.l -side left -anchor n
    pack $se -in $c_win.f$e_id -fill both -expand yes

    grid $c_win.esb_v -row $row -column [expr $column + 1] -sticky ns
    grid $c_win.esb_h -row [expr $row + 1] -column $column -sticky ew

    seqed_handle_bindings $c_win.f$e_id.l $c_win $se $e_id
}

proc seqed_handle_bindings {handle c_win e_win e_id } {

    bind $handle <<select>> "collapse_e_win $c_win $e_win $e_id $handle"
    bind $handle <B2-Leave> "handle_leave $e_win"
    bind $handle <Alt-Button-1> "handle_leave $e_win"
...
proc highlight_position_seqed {se y } {
    global e_from

    set pos [winfo rooty $se]
    set win_ht [winfo height $se]
    set ht2 [expr $win_ht / 2]
    set middle [expr $pos + $ht2]

    if {$y >= $pos && $y < $middle} {
	$e_from configure -cursor top_side
    } else {
	$e_from configure -cursor bottom_side
    }
