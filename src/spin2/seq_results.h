...
#define SEQ_TYPE_SPLICE_DONOR   (1<<9)
#define SEQ_TYPE_SPLICE_ACCEPTOR (1<<10)
#define SEQ_TYPE_WTMATRIXSEARCH (1<<11)
#define SEQ_TYPE_GRAPH_PLOT     (1<<12) /* emboss plots */
#define SEQ_TYPE_DOT_PLOT       (1<<13) /* sip/emboss plots */
#define SEQ_TYPE_RULER          (1<<14)
#define SEQ_TYPE_FT_VIEWER      (1<<15)

/* compound definitions */
#define SEQ_TYPE_GENESEARCH (SEQ_TYPE_CODONPREF | SEQ_TYPE_AUTHOR)
#define SEQ_TYPE_SPLICE (SEQ_TYPE_SPLICE_DONOR | SEQ_TYPE_SPLICE_ACCEPTOR)
...
typedef struct seq_result_ {
    ...
    int seq_id[5];      /* unique sequence identifier */
    int type;           /* unique name for each result type */
    int gr_type;        /* type of graphical plot */
 ...
 */
int Set_Seqs(int seq_num, int direction, int l_index, char *name,
	     char *sequence, int seq_structure, int seq_type,
	     Featcds **key_index, char *identifier);
...
d_point SpinFindNearestMatch(Graph *graph, d_point start, double x_scale);
d_point SpinFindNearestLine(Graph *graph, d_point start, double x_scale);
/*
