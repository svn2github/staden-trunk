			     ...
			     Tcl_Obj *CONST objv[]) 
{
	...
	{"-strand",    ARG_INT, 1, "1", offsetof(similar_spans_arg, strand)},
	{"-score",    ARG_INT, 1, "-1", offsetof(similar_spans_arg, score)},
	{NULL,     0,       0, NULL, 0}
    };
			   ...
			   Tcl_Obj *CONST objv[]) 
{
	...
	args.line_width = get_default_int(interp, sip_defs,
					   "SIP.CS.L_WIDTH");
    }

    if (strcmp(args.colour, "") == 0) {
	args.colour = get_default_string(interp, sip_defs, "SIP.CS.COLOUR");
    }
    
    if (-1 == init_sip_similar_spans_plot(interp, args.seq_id_h, 
					  args.seq_id_v, 
			  ...
			  Tcl_Obj *CONST objv[]) 
{
    ...
    if (strcmp(cmd, "create") == 0) {
      sip_similar_spans_create(clientData, interp, objc, objv);
    } else if (strcmp(cmd, "list") == 0) {
      sip_list(clientData, interp, objc, objv);
    } else if (strcmp(cmd, "plot") == 0) {
      sip_similar_spans_plot(clientData, interp, objc, objv);
    }
    return TCL_OK;
}
			    ...
			    Tcl_Obj *CONST objv[]) 
{
	...
	{"-strand",    ARG_INT, 1, "1", offsetof(align_seqs_arg, strand)},
	{NULL,     0,       0, NULL, 0}
    };
			    ...
			    Tcl_Obj *CONST objv[]) 
{
    ...
    if (strcmp(args.colour, "") == 0) {
	args.colour = get_default_string(interp, sip_defs, "SIP.ALIGN.COLOUR");
    }
    
    if (-1 == init_sip_global_align_plot(interp, args.seq_id_h, 
					  args.seq_id_v, 
					  args.result_id, args.element, 
					  args.element_id, args.container, 
					  args.container_id, args.results, 
			 ...
			 Tcl_Obj *CONST objv[]) 
{
    ...
    if (strcmp(cmd, "create") == 0) {
	sip_global_align_create(clientData, interp, objc, objv);
    } else if (strcmp(cmd, "list") == 0) {
	sip_list(clientData, interp, objc, objv);
    } else if (strcmp(cmd, "plot") == 0) {
	sip_global_align_plot(clientData, interp, objc, objv);
    }
    return TCL_OK;
}
			      ...
			      Tcl_Obj *CONST objv[]) 
{
	...
	{"-strand",    ARG_INT, 1, "1", offsetof(identity_arg, strand)},
	{NULL,     0,       0, NULL, 0}
    };
			    ...
			    Tcl_Obj *CONST objv[]) 
{
    ...
    if (args.line_width == -1) {
	args.line_width = get_default_int(interp, sip_defs,
					  "SIP.IDENTITY.LINE_WIDTH");
    }
    
    if (strcmp(args.colour, "") == 0) {
	args.colour = get_default_string(interp, sip_defs, "SIP.IDENTITY.COLOUR");
    }
     if (-1 == init_sip_matching_words_plot(interp, args.seq_id_h, 
					  args.seq_id_v, 
					  args.result_id, args.element, 
					  args.element_id, args.container, 
					  args.container_id, args.results, 
			   ...
			   Tcl_Obj *CONST objv[]) 
{
    ...
    if (strcmp(cmd, "create") == 0) {
      sip_matching_words_create(clientData, interp, objc, objv);
    } else if (strcmp(cmd, "list") == 0) {
      sip_list(clientData, interp, objc, objv);
    } else if (strcmp(cmd, "plot") == 0) {
      sip_matching_words_plot(clientData, interp, objc, objv);
    }
    return TCL_OK;
}
			      ...
			      Tcl_Obj *CONST objv[]) 
{
	...
	{"-strand",    ARG_INT, 1,    "1", offsetof(quick_scan_arg, strand)},
	{NULL,     0,       0, NULL, 0}
    };
			    ...
			    Tcl_Obj *CONST objv[]) 
{
    ...
    cli_args a[] = {
	{"-element",    ARG_STR,   1, NULL, offsetof(splot_arg, element)},
	{"-container", ARG_STR,   1, NULL, offsetof(splot_arg, container)},
	{"-container_id", ARG_INT, 1, NULL, offsetof(splot_arg, container_id)},
	{"-element_id", ARG_INT, 1, NULL, offsetof(splot_arg, element_id)},
	{"-seq_id_h",  ARG_INT, 1, NULL, offsetof(splot_arg, seq_id_h)},
	{"-seq_id_v",  ARG_INT, 1, NULL, offsetof(splot_arg, seq_id_v)},
	{"-result_id", ARG_INT,   1, NULL, offsetof(splot_arg, result_id)},
	{"-results",   ARG_OBJ,   1, NULL, offsetof(splot_arg, results)},
	{"-element_type", ARG_STR, 1, NULL, offsetof(splot_arg, element_type)},
	{"-width", ARG_INT, 1, "1", offsetof(splot_arg, line_width)},
	{"-fill", ARG_STR, 1, "", offsetof(splot_arg, colour)},
	{NULL,     0,       0, NULL, 0}
    };

    if (-1 == spin_parse_obj_args(a, &args, objc-1, &objv[1])) {
	verror(ERR_WARN, "sip best diagonals", "failure to parse input\n");
	return TCL_ERROR;
    }
    ...
    if (strcmp(args.colour, "") == 0) {
	args.colour = get_default_string(interp, sip_defs, "SIP.QS.COLOUR");
    }
   
    init_sip_best_diagonals_plot(interp, args.seq_id_h, args.seq_id_v, 
				 args.result_id, args.element, 
				 args.element_id, args.container, 
				 args.container_id, args.results, 
				 args.line_width, args.colour, 
			   ...
			   Tcl_Obj *CONST objv[]) 
{
    ...
    if (strcmp(cmd, "create") == 0) {
	sip_best_diagonals_create(clientData, interp, objc, objv);
    } else if (strcmp(cmd, "list") == 0) {
	sip_list(clientData, interp, objc, objv);
    } else if (strcmp(cmd, "plot") == 0) {
	sip_best_diagonals_plot(clientData, interp, objc, objv);
    }
    return TCL_OK;
}
			      ...
			      Tcl_Obj *CONST objv[]) 
{
	...
	{"-strand",       ARG_INT,   1, "1", offsetof(sim_arg, strand)},
	{NULL,     0,       0, NULL, 0}
    };
			 ...
			 Tcl_Obj *CONST objv[]) 
{
    ...
    if (strcmp(args.colour, "") == 0) {
	args.colour = get_default_string(interp, sip_defs, "SIP.SIM.COLOUR");
    }
    if (-1 == init_sip_local_align_plot(interp, args.seq_id_h, 
					args.seq_id_v, 
					args.result_id, args.element, 
					args.element_id, args.container, 
					args.container_id, args.results, 
			...
			Tcl_Obj *CONST objv[]) 
{
    ...
    if (strcmp(cmd, "create") == 0) {
      sip_local_align_create(clientData, interp, objc, objv);
    } else if (strcmp(cmd, "list") == 0) {
      sip_list(clientData, interp, objc, objv);
    } else if (strcmp(cmd, "plot") == 0) {
      sip_local_align_plot(clientData, interp, objc, objv);
    }
    return TCL_OK;
}
...
Sip_Init(Tcl_Interp *interp) {
    ...
    Tcl_CreateObjCommand(interp, "sip_similar_spans", tcl_sip_similar_spans, 
		      (ClientData) NULL, 
    ...
    Tcl_CreateCommand(interp, "sip_find_score", tcl_sip_find_score, 
		      (ClientData) NULL, 
		      (Tcl_CmdDeleteProc *) NULL);
    Tcl_CreateObjCommand(interp, "sip_global_align", tcl_sip_global_align, 
		      (ClientData) NULL, 
		      (Tcl_CmdDeleteProc *) NULL);
    Tcl_CreateObjCommand(interp, "sip_matching_words", tcl_sip_matching_words, 
		      (ClientData) NULL, 
		      (Tcl_CmdDeleteProc *) NULL);
    Tcl_CreateObjCommand(interp, "sip_best_diagonals", tcl_sip_best_diagonals, 
		      (ClientData) NULL, 
		      (Tcl_CmdDeleteProc *) NULL);
    ...
    Tcl_CreateCommand(interp, "get_remove_dup", GetRemoveDup, 
		      (ClientData) NULL, 
		      (Tcl_CmdDeleteProc *) NULL);
    Tcl_CreateObjCommand(interp, "sip_local_align", tcl_sip_local_align, 
			 (ClientData) NULL, 
			 (Tcl_CmdDeleteProc *) NULL);
    {
        char *s, c[10];
