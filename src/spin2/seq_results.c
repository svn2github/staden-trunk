 ...
 */ 
d_point SpinFindNearestLine(Graph *graph,
			d_point start,
			double x_scale)
{
    int i;
	...
	for (i = 0; i < graph->d_arrays[0].n_dlines; i++) {
	    x1 = graph->d_arrays[0].d_array[i].x0 / x_scale;
	    y1 = graph->d_arrays[0].d_array[i].y0;
	    x2 = graph->d_arrays[0].d_array[i].x1 / x_scale;
	    y2 = graph->d_arrays[0].d_array[i].y1;
	    start_x = start.x / x_scale;
		...
		if (dist < min) {
		    min = dist;
		    nearest_pt.x = graph->d_arrays[0].d_array[i].x0;
		    nearest_pt.y = graph->d_arrays[0].d_array[i].y0;
		}
	    } else {
		dist1 = sqrt(((start_x - x1) * (start_x - x1)) + 
			     ((start.y - y1) * (start.y - y1)));
	...
	for (i = 0; i < graph->p_arrays[0].n_pts; i++) {
	    x1 = graph->p_arrays[0].p_array[i].x / x_scale;
	    y1 = graph->p_arrays[0].p_array[i].y;
	    x2 = (graph->p_arrays[0].p_array[i+1].x + graph->p_arrays[0].p_array[i].x) / x_scale;
	    y2 = graph->p_arrays[0].p_array[i+1].y + graph->p_arrays[0].p_array[i].y;
		...
		if (dist < min) {
		    min = dist;
		    nearest_pt.x = graph->p_arrays[0].p_array[i].x;
		    nearest_pt.y = graph->p_arrays[0].p_array[i].y;
		}
	    } else {
		dist1 = sqrt(((start_x - x1) * (start_x - x1)) + 
			     ((start.y - y1) * (start.y - y1)));
...
d_point SpinFindNearestMatch(Graph *graph,
			 d_point start,
			 double x_scale)
{
    int i;
    ...
    for (i = 0; i < graph->p_arrays[0].n_pts; i++) {
	x = (start.x - graph->p_arrays[0].p_array[i].x)/x_scale;
	y = (start.y - graph->p_arrays[0].p_array[i].y);
#ifdef DEBUG
