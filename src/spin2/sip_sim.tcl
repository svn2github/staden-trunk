...
#
proc plot_local_align {seq_id_h seq_id_v res_id s_array num_seq_array res strand} {
	...
	sip_local_align plot -element $c_win$e_win\
		-container $c_win\
...
proc SimAlign2 {s range_h range_v strand num_align1 num_align2 match transition transversion start_gap cont_gap} {
    global HORIZONTAL VERTICAL DNA TOP_S BOTTOM_S
	...
	for {set i 0} {$i < $num_seq_array} {incr i} { 
	    set res [sip_local_align create \
		    -seq_id_h $seq_array($i,h) \
		    -seq_id_v $seq_array($i,v) \
	...
	if {$final_id != -1} {
	    ...
	    plot_local_align $seq_id_h $seq_id_v result_id seq_array $num_seq_array results $TOP_S
	...
	if {$final_id != -1} {
	    ...
	    plot_local_align $seq_id_h $seq_id_v result_id seq_array $num_seq_array results $BOTTOM_S
...
proc SimAlign { } {
    ...
    okcancelhelp $s.button -bd 2 -relief groove \
	    -ok_command "SimAlign2 $s $s.range_h $s.range_v \[strand_get $s.strand\]\
