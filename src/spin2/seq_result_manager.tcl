...
#generic calls from container.tcl
proc result_list_create {t help_cmd} {
    seq_result_list_create $t $help_cmd
}

proc result_list_update {args} {
    global tk_utils_defs

    #puts "******************** result_list_update  $args ********************"

    set t [keylget tk_utils_defs CONTAINER.RESULTS.WIN] 
    seq_result_list_update $t

    #container results menu
    if {$args != ""} {
	set c_win $args
	set c_info [get_container_info $c_win]
	set c_id [keylget c_info container_id]
	seq_result_menu_update $c_win [seq_result_names -container_id $c_id]
	seq_element_menu_update $c_win [seq_result_names -container_id $c_id]
    }
}
...
proc seq_result_list_create {t help_cmd} {
    ...
    #button $t.bb.h -text "Help" -command {show_help spin {SPIN-Result-Manager}}
    eval button $t.bb.h -text "Help" -command {$help_cmd}
...
proc seq_result_menu_update { c_win results } {
	...
	#puts "index $id"
	set colour [seq_result_info -index $id -option colour]
...
proc seq_element_menu_update { c_win results } {
    global nip_defs tk_utils_defs

    set mb [keylget tk_utils_defs CONTAINER.MENUBAR]
    
    set r $c_win$mb.[menu_path {Elements}]
...
proc seq_result_keybox_update {result_id results r} {
