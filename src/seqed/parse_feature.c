...
char *get_ft_entry ( FILE *fp, char *line, char **last_line ) {
    ...
    if (NULL == (loc_expr = (char *)xmalloc((MAX_SEQ_LINE) *sizeof(char))))    
	goto error;
    if (NULL == (tmp = (char *)xmalloc((MAX_SEQ_LINE) * sizeof(char))))
	goto error;
   
    strcpy(loc_expr, &line[5]); 
  
    /* to get next line to judge whether entry reading has finished or not */

    fgets(line, MAX_SEQ_LINE, fp); 
    while(line[5] == ' ' && strncmp(&line[0], "XX", 2)) {
	strcpy(tmp,&line[5]);
	loc_expr = (char *)xrealloc(loc_expr,strlen(loc_expr) + 1+ strlen(tmp) + 1);
	strcat(loc_expr, tmp); 
	fgets(line, MAX_SEQ_LINE, fp);
    }
    *last_line = line;
