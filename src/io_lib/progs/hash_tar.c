...
int main(int argc, char **argv) {
    ...
    h = HashTableCreate(0, HASH_DYNAMIC_SIZE);
    ...
    if (argc != 1) {
	fprintf(stderr, "Usage: hash_tar [-v] [-d] tarfile > tarfile.hash\n");
	return 1;
    }
	...
	size = strtoul(blk.header.size, NULL, 8);
		...
		hd.idx.pos = offset+sizeof(blk);
		hd.idx.len = size;
		hi = HashTableAdd(h, member, strlen(member), hd, NULL);
		if (verbose) {
		    fprintf(stderr, "Add '%s', pos %d, len %d\n",
			    member, (int)hd.idx.pos, (int)hd.idx.len);
		}
            }
        }
    ...
    HashTableStats(h, stderr);

    HashFileSave(h, stdout, argv[0]);
    HashTableDestroy(h);
