...
static void test_mode(GapIO *io, contig_t **c, int xpos) {
    ...
    r = (rangec_t *)items_in_range(io->store, contig_get_bin(c),
				   xpos, xpos+79, &nr);
    qsort(r, nr, sizeof(*r), sort_range);
    for (i = 0; i < nr; i++) {
	seq_t *s = get_seq(io, r[i].rec);
...
int main(int argc, char **argv) {
	...
	GapIO *child = gio_child(io);
	init_curses();
	curses_loop(child, &c, xpos, mode);
	endwin();
    ...
    if (!lp_mode) {
	printf("\n\n\tg_view:\tShort Read Alignment Viewer, version 0.11\n");
