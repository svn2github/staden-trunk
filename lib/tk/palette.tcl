...
# by Tk.
#
# RCS: @(#) $Id: palette.tcl,v 1.2 2004-06-30 14:29:29 jkbonfield Exp $
#
...
proc ::tk::RecolorTree {w colors} {
	...
	#jkb - this sometimes fails. Eg when configure returns
	# selectColor SelectColor {} {}
	catch {if {![catch {$w configure $option} value]} {
		...
		# jkb
	    	if {[lsearch -exact {Text Entry Listbox} \
		             [winfo class $w]] != -1 &&
		    $dbOption == "background"} {
		    set col [tk::Darken $c($dbOption) 115]
	        } else {
		    set col $c($dbOption)
	        }
		...
		# the same colors.
		append result ";\noption add [list \
		    *[winfo class $w].$dbOption $col 60]"
		$w configure $option $col
